...
!        integer,parameter:: mmx=10, nx=2**mmx !  nx>2
        integer,parameter:: mmx=8, nx=2**mmx !  nx>2
        integer,parameter:: mmy=8, ny=2**mmy !  ny>2
...
!************************************************
        ...
        real*8, dimension(nx+1,ny+1) :: qour,jco,jxo,jyo
        real*8, dimension(4) :: my1
        real*8, dimension(nx,ny) :: hexper0
        
        integer, dimension(3) :: my2
        integer:: i1,i2
...
!        end do
        ...
        call mainprogram(my1,my2,hexper0,qour,jco,jxo,jyo)
2       format(1x,7e14.5)
        open(1,file='OUT.dat')
        do j=1,my2(2)
!        do j=nyexp/4,nyexp*3/4
c~ !        do j=nyexp/20,nyexp*19/20

        do i=1,my2(1)
        i1=i+nx/2-my2(1)/2
        i2=j+ny/2-my2(2)/2
        write(1,2) x(i1),y(i2),qour(i1,i2),jxo(i1,i2),jyo(i1,i2)
        end do
        end do
        close(1)
        end program test
!**********************************************************************        
        subroutine mainprogram(my1,my2,hexper0,qour,jco,jxo,jyo)
        use aaa
        ...
        real*8, dimension(nx,ny) :: hexper0
 !       print*,' Nx, Ny ',nx,ny
        call init(dx,dy,my1,my2,hexper0)
 !       print*,' dh= ',dh
 !       print*,'  '
...
!        do j=nyexp/20,nyexp*19/20
        ...
        jco(i1,i2)=dsqrt(xxjx(i1,i2)**2+xxjy(i1,i2)**2)
        !print *, jco(i1,i2)
        end do
        end do
...
***********************************************************
        subroutine init(dx,dy,my1,my2,hexper0)
        use aaa
  ...
  !      print*,' AFM phaze probe '
        ...
        if(kind==3) then
  !      print*,' AFM empirical phase probe '
         exit
        end if
        stop ' kind error '
        end do
