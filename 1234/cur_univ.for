...
!********  base lay determination ***************
        d2x=(bz3d(ix-1,iy,iz)-2.d0*bz3d(ix,iy,iz)+bz3d(ix+1,iy,iz))/dx**2
        d2y=(bz3d(ix,iy-1,iz)-2.d0*bz3d(ix,iy,iz)+bz3d(ix,iy+1,iz))/dy**2
        d2z=(bz3d(ix,iy,iz)-2.d0*bz3d(ix,iy,iz) +bz3d(ix,iy,iz+1))/dz**2       
!********  End of base lay determination ***************
        else
        d2x=(bz3d(ix-1,iy,iz)-2.d0*bz3d(ix,iy,iz)+bz3d(ix+1,iy,iz))/dx**2
        d2y=(bz3d(ix,iy-1,iz)-2.d0*bz3d(ix,iy,iz)+bz3d(ix,iy+1,iz))/dy**2
        d2z=(bz3d(ix,iy,iz-1)-2.d0*bz3d(ix,iy,iz)+bz3d(ix,iy,iz+1))/dz**2
        end if
...
!        do j=nyexp/10,nyexp*9/10
        ...
        i1=i+nx/2-nxexp/2
        i2=j+ny/2-nyexp/2
        write(1,1) x(i1),y(i2), dsqrt(xxjx(i1,i2)**2+xxjy(i1,i2)**2)
        end do
        end do
        close(1)
...
!        integer,parameter:: mmx=10, nx=2**mmx !  nx>2
        ...
        do i=1,nxexp
        do j=1,nyexp
        s0=s0+(bztes(i+nx/2-nxexp/2,j+ny/2-nyexp/2)-bzold(i+nx/2-nxexp/2,j+ny/2-nyexp/2))**2
        s1=s1+(bztes(i+nx/2-nxexp/2,j+ny/2-nyexp/2))**2+(bzold(i+nx/2-nxexp/2,j+ny/2-nyexp/2))**2
        end do
        end do
        err=(s0/s1)
        end
